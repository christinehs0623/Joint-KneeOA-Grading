...
# coding: utf-8
"""
correlation_medial_lateral_ours_OARSI_KL.py

Minimal inference pipeline for the OARSI + KL model
...
For every patient in `test_pid.json` this script:
    ...
    2. Runs the model to get OARSI predictions (kl, jsnm, jsnl, osfm, ostm, ostl, osfl)
       and the MIL attention scores.
    3. Computes an *ensemble* GradCAM heatmap (mean of GradCAM, GradCAM++, ScoreCAM,
       AblationCAM, LayerCAM) on the predicted KL class.
    4. Counts how many of the 41 bonefinder patches are to the left vs. right of
       bonefinder point 92, and labels them as lateral/medial based on the
       _L / _R side (see `count_lateral_vs_medial`).
...
MODEL_PATH      = "quantus/models/ours_OARSI_KL.pth"
MEAN_STD_PATH   = "quantus/models/mean_std_oai.npy"
TEST_PIDS_JSON  = "test_pid.json"
SHAPES_NPZ      = "quantus/utils/id_shapeLR_V00.npz"

OUTPUT_DIR      = "inference"
OUTPUT_CSV      = os.path.join(OUTPUT_DIR, "inference_v2_results.csv")
OUTPUT_JSON     = os.path.join(OUTPUT_DIR, "inference_v2_results.json")

# OARSI + KL (matches the "all" multitask config in OARSI_folder/config.py)
OARSI_TASKS = { "kl":   5, "jsnm": 4, "jsnl": 4, "osfm": 4, "ostm": 4, "ostl": 4, "osfl": 4,}
TASK_NAMES = list(OARSI_TASKS.keys())
...
def load_mean_std(path: str):
    ...
    Handles all the shapes `np.load` may produce:
        * `[mean, std]` saved as object array (most common here)
        * a single 0-d / 1-element array of mean only
        * separate `.npy` files (not used here, but defensive)
    """
    ...
    try:
        ...
        # `np.save([mean, std])` -> 1-D object array of length 2
        if isinstance(data, np.ndarray) and data.dtype == object and data.shape == (2,):
            ...
            mean, std = data[0], data[1]
        else:  # single 0-d or 1-element array of mean only
            mean, std = data, None
        ...
        if s is None:
            print(f"[inference_v2] {path} did not contain a std; using fallback.")
            return fallback
        return m, s
    except Exception as e:
        print(f"[inference_v2] Could not load mean/std from {path}: {e}. Using fallback.")
        return fallback
...
def main():
    ...
    model = CompleteMILModel_MultiTask_imedslab(
        feature_extractor_out_dim=FEATURE_EXTRACTOR_OUT_DIM,
        oai_task_num_classes=OARSI_TASKS,
    ...
    model.eval()
    print(f"[correlation_medial_lateral] Loaded model weights from {MODEL_PATH}")
    ...
    for batch in test_loader:
        ...
        # Reset gradients before each forward (matches the original
        # `inference.py` line 2309).
        model.zero_grad()
        patch_bag_tensor = patch_bags[0].to(DEVICE)              # (N, 1, 16, 16)

        # 1) Forward pass -> OARSI predictions + attention scores
        outputs = model([patch_bag_tensor])
        if outputs is None:
            summary["skipped"] += 1
            continue
        oai_preds_all, att_scores, _patch_feats, _agg_feats = outputs

        # 2) Predicted KL class (used as the GradCAM target)
        target_class = int(torch.argmax(oai_preds_all["kl"][0]).item())

        # 3) Ensemble CAM (5 methods, mean) -- now USED below to weight
        ...
        # 4) OARSI task predictions
        preds = {t: int(torch.argmax(oai_preds_all[t][0]).item())
                 for t in TASK_NAMES}

        # 4b) Ground truth (already in the batch from KneeMILDataset)
        # _features is shape (1, 1, 6) with column order
        # [jsnm, jsnl, osfm, ostm, ostl, osfl].
        # -999 means "not graded" for that sub-score.
        def _to_gt(v):
            v = int(v)
        ...
        row = {
            ...
            # Raw pixel-level hot counts (16x16 per patch)
            "lateral_hot_count": lateral_hot_count,
            "medial_hot_count":  medial_hot_count,
            "hot_count_ml":      hot_count_ml,
            "dominant_side":     dominant,
            # Signed Medial - Lateral differences (model-side)
            "heatmap_ml":        round(heatmap_ml, 6),
            "jsn_ml":            preds["jsnm"] - preds["jsnl"],
            "osn_ml":            (preds["osfm"] + preds["ostm"])
                                  - (preds["osfl"] + preds["ostl"]),
            "pred_kl":           preds["kl"],
            "pred_jsnm":         preds["jsnm"],
            "pred_jsnl":         preds["jsnl"],
            "pred_osfm":         preds["osfm"],
            "pred_ostm":         preds["ostm"],
            "pred_ostl":         preds["ostl"],
            "pred_osfl":         preds["osfl"],
            # Ground truth (from the h5 file)
            "gt_kl":             gt_kl,
            "gt_jsnm":           gt_jsnm,
            "gt_jsnl":           gt_jsnl,
            "gt_jsn_ml":         gt_jsn_ml,
